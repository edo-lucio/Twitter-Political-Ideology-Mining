...
def remove_intersections(input_files):
    ...
    output_dir = Path(f"data-collection//data//users-list//no-intersection-pairs//{file_names[0] + '-' + file_names[1]}")
    output_dir.mkdir(exist_ok=True)
    output_files = [f"{output_dir}//{file_name}-followers-list.csv" for file_name in file_names]
